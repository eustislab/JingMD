###########################################################
### This script create a folder of inp for TDDFT with   ###
### user-defined time range and TDDFT functional        ###
###########################################################
...
try:
	if str(sys.argv[1])=='?':
		print '\nCall function as: fincut.py input.log numberOfSoluteAtoms numberofSolventAtoms numberOfSolventMolecules startTime stopTime timePerFrame Functional\n' 
		sys.exit()
except IndexError:
    print '\n!!!Input command Error. Call function as: fincut.py input.log numberOfSoluteAtoms numberofSolventAtoms numberOfSolventMolecules startTime stopTime timePerFrame Functional\n' 
    sys.exit()
...
try:
    ...
    Functional=str(sys.argv[8])
except IndexError:
    print '\n!!!Input command Error. Call function as: fincut.py input.log numberOfSoluteAtoms numberofSolventAtoms numberOfSolventMolecules startTime stopTime timePerFrame Functional\n' 
    sys.exit()

#LCBLYP has special file format
if Functional=='LCBLYP':
	FunctionalPrint='BLYP'
	LC=True
else:
	FunctionalPrint=Functional
	LC=False
...
path = os.getcwd()
inputPath=path+'/'+input+'InputFiles_'+Functional
...
for line in f:
    ...
    if shouldCollect(lineNumber):
        ...
        if shouldWrite(lineNumber):
            ...
            #write header
            headerString=""" $CONTRL SCFTYP=RHF TDDFT=EXCITE DFTTYP="""+FunctionalPrint+"""
       RUNTYP=ENERGY ICHARG=0 MULT=1 COORD=UNIQUE 
       MAXIT=200 NPRINT=-5 ISKPRP=1 $END   
 ...
 $TDDFT NSTATE=5 TPA=.f. $END\n"""
            if LC==False:
			        	headerString=headerString+' $DFT LC=.f. $END'
            else:
            	headerString=headerString+' $DFT LC=.t. $END'
            headerString=headerString+"""\n $BASIS GBASIS=N311 NGAUSS=6 NDFUNC=2 NPFUNC=1 
        DIFFSP=.TRUE. DIFFS=.TRUE. POLAR=POPN311 $END             
 $DATA\n"""+input+' at t= '+str(time)+'\nC1 1\n'
            print 'Making input file for '+input+' at t= '+str(time) +' with '+Functional
            #write the header
...
f = open('megaio_'+input+'_hpc.txt','w')
program = 'gmssub'
processors = '32'
...
for item in allFiles:
	...
	f.write(program + ' %s '%item + processors +' '+outputName+' -l virtual_free=4g\n')

#for moving megaio to current folder and set permission to read
cmdstring = 'chmod 777 megaio_'+input+'_hpc.txt\n'+'mv megaio_'+input+'_hpc.txt '+inputPath
os.system(cmdstring)

#uncomment if want to call for xsedeGenerator.py
#os.system('python xsedeShellGenerator.py '+input+'InputFiles_'+Functional)